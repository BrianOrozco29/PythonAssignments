...
def boardSetup(boardSize, numTrapdoors, numCatapults):
    ...
    while len(catapults) < numCatapults:
        catapultSpace = random.randint(1, boardSize-2)
        if catapultSpace in trapdoors:
            continue

        destination = random.randint(1, 97)
        while destination in trapdoors:
            destination = random.randint(1, 97)

        catapults[catapultSpace] = destination
    ''' PROBLEM 3. Return the trapdoors catapults dictionaries '''

    #Solution 3
    return {'trapdoors':trapdoors, 'catapults':catapults}
...
def playGame():
    ...
    bothPlayers = getPlayerNames()
    player1, player2 = bothPlayers['player1'], bothPlayers['player2']
    
    ''' PROBLEM 4. Call boardSetup() to get the two dictionaries giving the
        spacedestination pairs for trapdoors and catapults. Look at the
        parameters in the boardSetup function to determine what arguments
        need to be passed to it. '''
    
    #Solution 4
    spacePairs = boardSetup(boardSize, numTrapdoors, numCatapults)
    trapdoors = spacePairs['trapdoors']
    catapults = spacePairs['catapults']
    ...
    while True:
        ...
        if position[currentPlayer] in catapults:
            ...
            print('Catapult!', currentPlayer, 'is fired to', position[currentPlayer])
            currentPlayer = switchPlayer(currentPlayer, player1, player2)
            continue
